...
interface DTableProps extends React.Props<any> {
    topLinks: Array<any>, 
    pageVolume: number, 
    clickCell: (col:number, row:number, elment: any) => void,
    HEADER_INFO: Array<any>};
interface DTableState extends React.StatelessComponent<any> {selectedIndex: number};
...
export class DTable extends React.Component<DTableProps, DTableState > {
  ...
  render() {
      ...
      const {topLinks, pageVolume, clickCell, HEADER_INFO} = this.props
...
export const DefaultTable: React.StatelessComponent<Props> = (props) => {
    const tableState = {
        fixedHeader: true,
        fixedFooter: true,
        stripedRows: false,
    ...
    const {selectedIndex, pageVolume, topLinks, clickCell, clickPage, HEADER_INFO} = props;
